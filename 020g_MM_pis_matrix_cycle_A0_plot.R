...
folderSim = paste0("C:/Users/2024ar003/Desktop/Alcuni file permanenti/Post_doc/Dati/DRIAS", mod, "_sim_020e")
folderPlot = paste0("C:/Users/2024ar003/Desktop/Alcuni file permanenti/Post_doc/Esperimenti/Outputs/Scenari climatici/DRIAS", mod, "_sim_020e")

nIDs = 8981 # number of regions
IDs = 1:nIDs
IDsSubSet = IDs

files = list.files(paste0(folderSim,"/"), pattern = "A0_")
...
A01 <- readRDS(paste0(folderSim, "/", files[1]))

A0m = matrix(NA, ncol = length(A01), nrow = length(files))

for (i in 1:length(files)){
  file = files[i]
  A0v <- readRDS(paste0(folderSim, "/", file))
  A0m[i,]= A0v
}
...
A0MM <-  matrix(NA, ncol = nIDs, nrow = nrow(scenariosDF))
...
for(k in 1:nrow(scenariosDF)){
  ...
  id0nas = rbind(which(A0mSel==0, arr.ind = TRUE),which(is.na(A0mSel), arr.ind = TRUE))
  
  A0mSelcorr = A0mSel
  
  for(i in 1:nrow(id0nas)){
    coli <- A0mSel[,id0nas[i,2]]
    colicr <- coli[which(coli>0 & !is.na(coli))]
    A0mSelcorr[id0nas[i,1], id0nas[i,2]]= exp(mean(log(colicr)))
  }
  
  A0mSelcorr[is.na(A0mSelcorr)] = 10^-10
  
  A0Sel <-  apply(A0mSelcorr, 2,
                  function(x){exp(mean(log(x), na.rm = T))})
  
  
  A0MM[k,] =  A0Sel
  
  A0SelCut <- cut(A0Sel, breaks=cutPal,
                  labels=sapply(cutPal [-length(cutPal )], function(x){paste0(">", as.character(x))}))
  
  plotCut <- ggplot()+
    geom_sf(data = domain, aes(fill = A0SelCut), colour = NA)+ #
    scale_fill_manual(values = colPal)+
    ggtitle(paste0("A0, ", name, ", ", min(years), "-", max(years)))+
