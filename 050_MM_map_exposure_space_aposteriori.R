...
plotCut <- ggplot() +
  ...
  geom_path(data = IndDT, aes(x = tasMaxSum, y = tasMinWin, group = city), #AvgYea
            arrow = arrow(ends = "both", type = "closed", length = unit(0.05, "inches")), color = "grey70") + # , color= "white"
  
  geom_point(data = IndDT, aes(x =tasMaxSum, y = tasMinWin, shape = scenario, color = statusSuitability), size = 4) + #, color= "white", #shape = MapDT$pointShape, 
