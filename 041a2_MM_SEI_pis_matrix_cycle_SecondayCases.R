...
source("041b2_MM_SEI_integration_functions.R")
...
for (year in years){
  ...
  parmsInv = list(omega = omega[nrow(omega):1,, drop = FALSE],
                  h = h[nrow(h):1,, drop = FALSE],
                  K = KM2[nrow(KM2):1,, drop = FALSE],
                  muA = muA[nrow(muA):1,, drop = FALSE],
                  deltaE = deltaE,
                  sigma = sigma[nrow(sigma):1,, drop = FALSE],
                  gamma = gamma,
                  tasMax = tasMax[nrow(tasMax):1,, drop = FALSE],
                  tasMin = tasMin[nrow(tasMin):1,, drop = FALSE],
                  nIDs = nIDs,
                  tSr = tSr[nrow(tSr):1,, drop = FALSE],
                  A = A[nrow(A):1,, drop = FALSE],
                  phiA = phiA[nrow(phiA):1,, drop = FALSE],
                  bH2v = bH2v,
                  bv2H = bv2H,
                  deltaM = deltaM,
  ...
  ## Save results ----
  saveRDS(Adults, file = paste0(folderOut, "/041a2_Adults_Drias_SEIS_", name, "_", year, ".rds"))
  saveRDS(SH, file = paste0(folderOut, "/041a2_SH_Drias_SEIS_", name, "_", year, ".rds"))
