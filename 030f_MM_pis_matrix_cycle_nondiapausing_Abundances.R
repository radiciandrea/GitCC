...
if (file.exists("C:/Users/2024ar003/Desktop/Alcuni file permanenti/Post_doc/Codice/local.R")){
  ...
  folderOut = paste0("C:/Users/2024ar003/Desktop/Alcuni file permanenti/Post_doc/Dati/DRIAS",mod,"_sim_030f")
} else {
...
J0 = rep(0, nIDs)
I0 = rep(0, nIDs)
A0 = 1*rep(1, nIDs) # at the 1st of July
Ed_0 = rep(0, nIDs) 
...
for (year in years){
  ...
  ### Split and re-merge weather
  omegaRev = 0*omega
  hRev = h[c(FoJul:nD, 1:(FoJul-1)),, drop = FALSE]
  KM2Rev = KM2[c(FoJul:nD, 1:(FoJul-1)),, drop = FALSE]
  ...
  SimRev = cbind(SimLog1[,1], 10^4*(exp(SimLog1[, 1+1:(nIDs*5)])-1))
  ...
  #extract Adults
  Adults <- Sim[,1+3*nIDs + 1:nIDs, drop = FALSE]
  
  ## Save results ----
  saveRDS(Adults, file = paste0(folderOut, "/030f_Adults_Drias_", name, "_", year, ".rds"))
